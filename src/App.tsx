...
function ChatPage() {
  ...
  return <main className="chat-app">
    ...
    <section className="conversation terminal-mode">
      <header className="chat-header"><button className="mobile-menu" onClick={() => setMenuOpen(true)} aria-label="Open navigation"><Menu size={20} /></button><div className="terminal-titlebar"><span className="terminal-lights"><i /><i /><i /></span><Link to="/" className="model-picker"><span className="model-orb"><Sparkles size={14} /></span><div><strong>ouagx://session</strong><small>{modelLabel}</small></div><ChevronDown size={16} /></Link></div><div className="terminal-status"><span className={ready ? 'live-dot' : 'idle-dot'} />{ready ? 'online' : 'offline'}<button className="header-control" onClick={() => setSettingsOpen(true)}><KeyRound size={17} /> <span>Connections</span></button></div></header>
      <div className="thread">{messages.map((message, index) => <article className={`message ${message.role}`} key={`${message.role}-${index}`}>
        ...
        <div className="message-body"><div className="message-name">{message.role === 'assistant' ? 'ouagx@core:~$' : 'you@local:~$'}</div><p>{message.content}</p>{message.role === 'assistant' && index > 0 && <button className="copy-answer" onClick={() => copyResponse(message.content)}>{copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'Copied' : 'Copy output'}</button>}</div>
      ...
      <div className="composer-zone"><div className="suggestions"><button onClick={() => setPrompt('Help me turn a rough idea into a focused plan.')}>plan</button><button onClick={() => setPrompt('What should I focus on today?')}>focus</button><button onClick={() => setPrompt('Challenge my assumptions about this idea: ')}>challenge</button></div><form className="composer" onSubmit={send}><span className="shell-prompt">$</span><textarea ref={composer} value={prompt} onChange={event => setPrompt(event.target.value)} placeholder={ready ? 'type a command...' : 'connect a model first...'} rows={1} /><button className="send-button" aria-label="Run command" disabled={!prompt.trim() || isSending}><ArrowUp size={19} /></button></form><p className="privacy-note"><Globe2 size={12} /> session encrypted locally // provider requests direct</p></div>
    </section>
